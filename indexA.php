<?php get_header(); ?>
<!-- 博客样式 -->
<!-- 内容开始 -->
<div class="container">
<div class="row">

	<!--main begin-->
	<div class="col-md-9 col-sm-12">
	<div class="main global" >
		<?php if(isIndex() == true) :?>
		<!--幻灯开始-->
		<div class="tit">
			<h1>热门推荐</h1> 
		</div>
		<?php include(TEMPLATEPATH . '/slider.php'); ?>
		<!--幻灯结束-->
		<?php endif ;?>
		<!-- 最新文章开始 -->
		<div class="tit">
			<h1>最新文章</h1>
		</div>
		<div id="posts">
		<?php $posts =  query_posts($query_string .'&ignore_sticky_posts=1&orderby=date&showposts=10');?>       
		<?php if( $posts ) : ?>                                      
			<?php while ( have_posts() ) : the_post() ;?> 
			<div id="post-<?php the_ID() ?>"  class="onepost content">
			<div class="row">
				<div class="col-md-4 hidden-xs">
					<div class="thumb">
						<a  href="<?php the_permalink() ?>"  title="<?php the_title(); ?>"  >
						<img data-original="<?php post_thumbnail_src();?>"  alt='<?php the_title(); ?>' class="lazy img img-responsive img-rounded">
						</a>
					</div>
				</div>
				<div class="col-md-8">
					<dl>
						<div class="title">
							<a title="<?php the_title(); ?>"  href="<?php the_permalink() ?>" ><?php the_title(); ?></a>
						</div>
						  
						<dd>
							<p><?php echo mb_strimwidth(strip_tags($post->post_content), 0,350,"..."); ?></p>
						</dd>
						<div class="clearfix"></div>
						<div class="info hidden-xs">
					          	<span class="mlabel-success">分类:</span>&nbsp;<span class="cate"><?php the_category(' '); ?> </span>
					          	<div class="views"><span class="mlabel-info"><a href="<?php the_permalink() ?>" title="<?php the_title(); ?>">View More</a></span></div>
						 </div>
					</dl>
				</div>
			</div>
			</div>
			<?php endwhile; ?>                                          
		<?php endif; ?>	
		</div>
	</div>	
	<div class="clearfix"></div>
	<div class="ajax_btn">
		<!--动态载入-->
		<?php if ( $wp_query->max_num_pages > 1 ): ?>
		<div class="aload">
			<?php next_posts_link( __( '点击载入更多内容', 'bubu' ) ); ?>
		</div>
		<?php endif; ?>
	</div>			

		<!-- 最新文章结束 -->


	</div>
	<!--main end-->
	
	<!--sidebar begin-->
	<div class="col-md-3 hidden-sm hidden-xs global">
		<?php get_sidebar("index"); ?>
	</div>
	<!-- sidebar end -->
</div>

</div>
<!-- 内容结束 -->

<?php get_footer(); ?>
